...
**Average proxy cost: 1.2237** across 17 ICCAD04 IBM benchmarks
- All 17 benchmarks: 0 overlaps
- 17% better than the 1.4738 baseline
- 16% better than RePlAce (1.4578)
- Runtime: ~20-50 min per benchmark on RTX 4050 laptop
...
- **Differentiable Congestion:** Soft L-routing demand accumulation using sigmoid-based range indicators, box-filter smoothing matching TILOS's smooth_range=2, and soft ABU5 via detached quantile threshold
...
**Optimizer:** Adam with cosine annealing learning rate schedule. Two-pass strategy:
- Coarse pass (lr=0.005, 200s): find the right basin
- Fine pass (lr=0.0008, 400s): polish within the basin

Each pass is followed by legalization to fix any residual overlaps.

**Result on ibm01:** Starting from DREAMPlace output (proxy 0.93), the differentiable optimizer reduces it to 0.878 — a 5.6% additional improvement that DREAMPlace alone cannot achieve.

### Stage 3: ABU5 Congestion Shifter

After the differentiable optimizer, we run a targeted congestion pass:
1. Extract the congestion tile map from the TILOS evaluator
2. Identify the top-5% hotspot tiles (these determine the ABU5 score)
3. Find macros near those hotspots
4. Try moving them toward cooler regions
5. Accept moves that reduce the actual TILOS proxy

This directly attacks the specific tiles that determine the congestion score.
...
| Differentiable optimizer | Gets us to ~1.22 (directly optimizes TILOS proxy) |
| ABU5 shifter | Small additional gains on high-congestion benchmarks |
...
Checkpoint scan:                         ~60s
Differentiable optimizer (coarse+fine): ~600s
ABU5 shifter:                           ~60s
Cleanup:                                 ~5s
Total:                                  ~900-1700s (15-28 min)
```

On the eval server (RTX 6000 Ada, 3-4× faster): ~5-10 min per benchmark. Well within the 1-hour limit.
