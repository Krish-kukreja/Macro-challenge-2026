...
class PlaceDB (object):
    ...
    def initialize_from_rawdb(self, params):
        ...
        if pydb.num_routing_grids_x:
            ...
            # Bookshelf doesn't provide per-layer capacities; the routability
            # optimization (routability_opt_flag=1) needs the plural form.
            # Synthesize a single-layer array so routability_opt works.
            self.unit_horizontal_capacities = np.array(
                [params.unit_horizontal_capacity], dtype=self.dtype
            )
            self.unit_vertical_capacities = np.array(
                [params.unit_vertical_capacity], dtype=self.dtype
            )
            # Initial demand maps default to zero (no fixed routing in Bookshelf).
