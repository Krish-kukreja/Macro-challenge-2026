...
except ImportError:  # eval Docker may lack matplotlib; only used for plotting
    plt = None
import inspect
...
# torch_optimizer / ncg_optimizer are only required for non-default optimizers
# (AggMo, QHAdam, Yogi, RAdam, AdaBelief, etc.). Default 'nesterov' / 'adam'
# / 'sgd' don't need them. Make these imports lazy so the bundle stays
# self-contained on environments without these packages installed.
try:
                ...
                for Lgamma_step in range(model.Lgamma_iteration):
                    ...
                    for Llambda_density_weight_step in range(model.Llambda_density_weight_iteration):
                        ...
                        for Lsub_step in range(model.Lsub_iteration):
                            ...
                            # ─── Checkpoint hook ────────────────────────────
                            # If DREAMPLACE_CHECKPOINT_DIR env var is set, dump
                            # positions every DREAMPLACE_CHECKPOINT_EVERY iters
                            # so an external script can find the best-proxy
                            # iteration after the run finishes.
                            try:
                                _ckpt_dir = os.environ.get("DREAMPLACE_CHECKPOINT_DIR")
                                if _ckpt_dir:
                                    _ckpt_every = int(os.environ.get(
                                        "DREAMPLACE_CHECKPOINT_EVERY", "50"
                                    ))
                                    # Dump node names + scale/shift once at iteration 1
                                    # so the post-processor can map positions
                                    # → benchmark macros without parsing the log.
                                    if iteration == 1:
                                        os.makedirs(_ckpt_dir, exist_ok=True)
                                        _meta_path = os.path.join(_ckpt_dir, "meta.npz")
                                        if not os.path.exists(_meta_path):
                                            try:
                                                _names = np.array(
                                                    placedb.node_names[: placedb.num_movable_nodes]
                                                ).astype(np.str_)
                                            except Exception:
                                                _names = np.array([], dtype=np.str_)
                                            try:
                                                _scale = float(params.scale_factor)
                                                _shift = np.array(
                                                    params.shift_factor, dtype=np.float64
                                                )
                                            except Exception:
                                                _scale = 1.0
                                                _shift = np.array([0.0, 0.0])
                                            np.savez(
                                                _meta_path,
                                                node_names=_names,
                                                num_movable_nodes=placedb.num_movable_nodes,
                                    ...
                                    if iteration % _ckpt_every == 0:
                                        os.makedirs(_ckpt_dir, exist_ok=True)
                                        _pos_np = self.pos[0].data.detach().cpu().numpy()
                                        _ovf = (
                                            float(overflow_list[-1])
                                            if len(overflow_list) > 0 else -1.0
                                        )
                                        _ckpt_path = os.path.join(
                                            _ckpt_dir,
                                            f"ckpt_{iteration:06d}.npz",
                                        )
                                        np.savez(
                                            _ckpt_path,
                                            pos=_pos_np,
                                ...
                                # Never let checkpointing kill the optimizer
                                logging.warning(
                                    f"checkpoint hook failed: {_ckpt_err}"
                                )
